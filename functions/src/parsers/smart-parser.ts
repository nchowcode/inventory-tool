...
export class SmartParser {
  private readonly vendorPatterns: VendorPattern[] = [
    {
      ...
      domains: ["amazon.com"],
      patterns: {
        orderNumber: [/Order #\s*(\d{3}-\d{7}-\d{7})/i],
        total: [/Order Total:\s*\$\s*([\d,]+\.\d{2})/i],
        item: [
          /Your Amazon\.com order of (\d+) x "([^"]+)"/i, // Match quantity and item in subject
          /Your Amazon\.com order of "([^"]+?)\.{3}?"*/i,
        ],
    ...
    },
    {
      ...
      patterns: {
        ...
        item: [/Style:\s*(.*?)(?=Size:|$)/m],
        quantity: [/Quantity:\s*(\d+)/i, /QTY:\s*(\d+)/i],
        price: [/Price:\s*\$\s*([\d,]+\.\d{2})/i, /\$\s*([\d,]+\.\d{2})/],
      },
    },
  ];
  ...
  private readonly commonPatterns = {
    ...
    price: [/\$\s*([\d,]+\.\d{2})/, /price:?\s*\$\s*([\d,]+\.\d{2})/i],
    quantity: [/qty:?\s*(\d+)/i, /quantity:?\s*(\d+)/i, /×\s*(\d+)/],
  };
